...
void PETDetectorConstruction::BuildMaterial() {
  ...
  G4double reflectivity_pmt[4] = {100,100,100,100};
  G4double efficiency_pmt[4]   = {1.0,1.0,1.0,1.0};

  fLYSO_MPT->AddProperty("REFLECTIVITY", ephoton_pmt, reflectivity_pmt, 4);
...
//  G4double block_dX = 30*mm, block_dY = 30*mm, block_dZ = 20*mm;
  G4double cryst_dx = 25.8*mm, cryst_dy = 25.8*mm, cryst_dz = 10*mm;
  ...
  G4Tubs* solidIRing =
    new G4Tubs("RingI", ring_R1, ring_R2, 0.5*cryst_dx, 0., twopi);
    G4LogicalVolume* logicIRing =
      new G4LogicalVolume(solidIRing, fAir,"RingI");
  G4Tubs* solidIIRing =
    new G4Tubs("RingII", ring_R1+4*(cryst_dz), ring_R2+12*(cryst_dz), 0.5*cryst_dx, 0., twopi);
    G4LogicalVolume* logicIIRing =
      new G4LogicalVolume(solidIIRing, fLYSO,"RingII");

  G4Tubs* solidGel_outer = new G4Tubs("gel", 0, cryst_dz, 0.5*cryst_dx , 0, dPhi);
  G4LogicalVolume* logicGel_outer = new G4LogicalVolume(solidGel_outer,fGel,"GelLV");
  ...
  G4Box* solidDet_outter = new G4Box("detector",  det_dx/2, det_dy/2, det_dz/2);
  ...
  /*for (G4int icrys = 0; icrys < 3 ; icrys++) {
    G4double phi = icrys*dPhi;
    G4RotationMatrix rotm  = G4RotationMatrix();
    rotm.rotateX(90*deg);
    rotm.rotateZ(phi);
    G4ThreeVector uz = G4ThreeVector(std::cos(phi),  std::sin(phi),0);
    G4ThreeVector position = G4ThreeVector((ring_R1)*std::cos(phi+dPhi), (ring_R1)*std::sin(phi+dPhi)-25.8*mm,-12.9*mm); //(ring_R1+0.5*cryst_dz)*uz
    G4Transform3D transform = G4Transform3D(rotm,position);

    new G4PVPlacement(transform,             //rotation,position
                      logicGel_outer,            //its logical volume
                      "gel",             //its name
                      logicIRing,             //its mother  volume
                      false,                 //no boolean operation
                      icrys);                // super secret number only I know
  }*/
  ...
  for (G4int icrys = 0; icrys < 1 ; icrys++) {
    ...
    rotm.rotateZ(phi);
    G4ThreeVector uz = G4ThreeVector(std::cos(phi),  std::sin(phi),0);
    G4ThreeVector position = G4ThreeVector((ring_R1+0.5*cryst_dz)*std::cos(phi), (ring_R1+0.5*cryst_dz)*std::sin(phi), 0); //(ring_R1+0.5*cryst_dz)*uz
    G4Transform3D transform = G4Transform3D(rotm,position);
  ...
  G4int By =  (std::sin(phi))*(ring_R1+0.5*cryst_dz);
  G4int ODx = (std::cos(phi))*(ring_R1+cryst_dz);
  G4int ODy =  (std::sin(phi))*(ring_R1+cryst_dz);
  G4int IDx = (std::cos(phi))*(ring_R1-0.5*cryst_dz);
  G4int IDy =  (std::sin(phi))*(ring_R1-0.5*cryst_dz);

   G4RotationMatrix rotm  = G4RotationMatrix();
    rotm.rotateY(90*deg);
    rotm.rotateZ(0.934*phi);
    for (G4int icrys = 0; icrys < 8 ; icrys++){
    for (G4int jcrys = 0; jcrys < 8 ; jcrys++){
    G4int OutdetCopyNumber = jcrys + 8*icrys + 64;
    G4ThreeVector position_cryst = G4ThreeVector(ODx+det_dz- (3.5-icrys)*1.13*det_dx*sin(phi),//ODx - (3.5-icrys)*det_dx*sin(phi),
    ...
    G4ThreeVector position_det_outter = G4ThreeVector(ODx,ODy,-cryst_dx/2); //Issue //wrong //unknown
  ...
  G4int Bx = (std::cos(phi))*(ring_R1+0.5*cryst_dz);
  G4int By =  (std::sin(phi))*(ring_R1+0.5*cryst_dz);
  G4int ODx = (std::cos(phi))*(ring_R1+1.5*cryst_dz);
  G4int ODy =  (std::sin(phi))*(ring_R1+1.5*cryst_dz);
  G4int IDx = (std::cos(phi))*(ring_R1-0.5*det_dz);
  G4int IDy =  (std::sin(phi))*(ring_R1-0.5*det_dz);
   ...
   G4RotationMatrix rotm  = G4RotationMatrix();
    ...
    for (G4int icrys = 0; icrys < 8 ; icrys++){
    for (G4int jcrys = 0; jcrys < 8 ; jcrys++){
      G4int IndetCopyNumber = jcrys + 8*icrys;
    G4ThreeVector position_cryst = G4ThreeVector(IDx- (3.5-icrys)*det_dx*sin(phi)+0.4*cos(phi)*mm,//ODx - (3.5-icrys)*det_dx*sin(phi),
  IDy+(3.5-icrys)*1.07*det_dx*cos(phi)+0.4*sin(phi)*mm,
  ...
  for (G4int imoda = 0; imoda < 1 ; imoda++){
    G4RotationMatrix rotm  = G4RotationMatrix();
    //rotm.rotateZ(-9*deg);
    G4double phi = imoda*dPhi;

    G4ThreeVector position_moda = G4ThreeVector(ring_R1+0.5*cryst_dz, 0, (25.78*imoda));
      G4Transform3D transform_moda = G4Transform3D(rotm,position_moda);
  new G4PVPlacement(transform_moda,                     //no rotation
...
/*  new G4PVPlacement(0,                     //no rotation
                    ...
                    logicIIRing,             //its logical volume
                    "IRing",                //its name
                    worldLog,         //its mother  volume
                    false, 0);                //no boolean operation

*/
        logicIRing->SetVisAttributes (G4VisAttributes::Invisible);
        logicIIRing->SetVisAttributes (G4VisAttributes::Invisible);
       ...
       logicDet_outer->SetVisAttributes(DetVisAttributes);
        ...
        logicGel_outer->SetVisAttributes(GelVisAttributes);
