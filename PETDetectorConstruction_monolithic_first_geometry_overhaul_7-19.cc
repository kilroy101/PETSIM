...
G4VPhysicalVolume * PETDetectorCon  //world_hxx, world_hy, world_hzstruction::Construct() {
  BuildMaterial();
...
//  G4double block_dX = 30*mm, block_dY = 30*mm, block_dZ = 20*mm;
  G4double cryst_dx = 25.8*mm, cryst_dy = 25.8*mm, cryst_dz = 15*mm;
  G4int nb_cryst =40;
  //G4int nb_crys = 8;
  //G4int nb_rings = 8;
  //
  ...
  //
  G4double detector_dZ = nb_rings*cryst_dx;//unknown
  ...
  G4Box* solidDet_outter = new G4Box("detector", cryst_dx/2, cryst_dy/2, cryst_dz/6);

  G4LogicalVolume* logicDet_outter =
    new G4LogicalVolume(solidDet_outter,fAir,"detector");

  G4Box* solidDet_inner = new G4Box("detector", cryst_dx/2, cryst_dy/2, cryst_dz/6);
